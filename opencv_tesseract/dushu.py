...
def img_sobel_binary(im, blur_sz):
    ...
    sobely = cv2.Sobel(blur_gray,cv2.CV_16S,0,1,ksize=3)
    abs_sobelx = np.absolute(np.sqrt(sobelx**2+sobely**2))
...
def img_contour_extra(im):
    ...
    # 获得连通区域
    # 该函数会破坏原始参数
    contours, hierarchy = cv2.findContours(bgmask.copy(), cv2.RETR_EXTERNAL, #外部轮廓
...
def img_tesseract_detect(c_rect, im):
    ...
    pts = np.array(c_rect).reshape(4, 2)
...
if __name__ == "__main__":
    ...
    F1 = "172_79.jpg"
    F2 = "633_88.jpg"
    F3 = '/Users/hzqb_luke/Desktop/Test/test_openCV/cardNumber/cards/test4.jpg'
